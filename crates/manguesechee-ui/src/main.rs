...
type TrayHandle = Option<ksni::blocking::Handle<tray::ManguesecheeTray>>;
...
fn main() -> anyhow::Result<()> {
    ...
    setup_lifecycle(&window);

    let tray = tray::ManguesecheeTray::new(window.as_weak());
    let tray_handle = tray.spawn_tray();

    init_app_state(&window);

    if let Ok(cfg) = config::load() {
        populate_settings_from_config(&window, &cfg);
    }

    setup_theme_and_toast(&window);
    setup_modals(&window);
    setup_service_controls(&window);
    setup_wizard(&window);
    setup_settings(&window);
    setup_topology(&window);
    setup_logs(&window);
    setup_cursor_and_edge(&window, tray_handle.clone());
    setup_peers(&window);
    setup_connection_controls(&window, tray_handle.clone());

    let _poll_timer = setup_polling_timer(&window, tray_handle);

    window.run()?;
    Ok(())
}

fn setup_lifecycle(window: &MainWindow) {
...
fn init_app_state(window: &MainWindow) {
...
fn setup_theme_and_toast(window: &MainWindow) {
    let w = window.as_weak();
...
fn setup_modals(window: &MainWindow) {
    let w = window.as_weak();
...
fn setup_service_controls(window: &MainWindow) {
    let w = window.as_weak();
...
fn setup_wizard(window: &MainWindow) {
    let w = window.as_weak();
...
fn setup_settings(window: &MainWindow) {
    let w = window.as_weak();
...
fn setup_topology(window: &MainWindow) {
    let w = window.as_weak();
...
fn setup_logs(window: &MainWindow) {
    let w = window.as_weak();
...
fn setup_cursor_and_edge(window: &MainWindow, tray_handle: TrayHandle) {
    let w = window.as_weak();
    let th = tray_handle;
...
fn setup_peers(window: &MainWindow) {
    let w = window.as_weak();
...
fn setup_connection_controls(window: &MainWindow, tray_handle: TrayHandle) {
    let w = window.as_weak();
...
fn setup_polling_timer(window: &MainWindow, tray_handle: TrayHandle) -> slint::Timer {
    let w = window.as_weak();
    let th = tray_handle;
        ...
        move || {
            ...
            }) = poll_status()
            {
                ...
                if let Some(p) = connected_to {
                    w.set_connected_peer(p.into());
                }

                w.set_topology_configured(topology_configured);
                ...
                let entries: Vec<PeerEntry> = peers.iter().map(|p| {
                    ...
                    PeerEntry {
                        ...
                        ping_status:  "".into(),
                        position:     p.position.clone().into(),
                        grid_x:       p.grid_x,
                        grid_y:       p.grid_y,
                    }
                }).collect();
