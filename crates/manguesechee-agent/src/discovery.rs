...
use manguesechee_network::{DiscoveredPeer, Discovery};
            ...
            loop {
                ...
                if !discovered.is_empty() {
                    ...
                    for peer in discovered {
                        ...
                        resolve_name_collision(&peer, &name, &id, port, &mut s, &disc, &broadcast_tx);
                        register_or_update_discovered_peer(&peer, &mut s);
                    }
                }
...
fn resolve_name_collision(
    ...
    id: &str,
    port: u16,
    state: &mut crate::ipc_server::AgentState,
    disc: &Discovery,
    broadcast_tx: &tokio::sync::broadcast::Sender<manguesechee_core::protocol::Message>,
) {
    let current_local_disp = state.local_display_name.clone();
        ...
        && peer.display_name.eq_ignore_ascii_case(&current_local_disp)
    {
        ...
        if id < peer.id.as_str() {
            ...
            state.local_display_name = new_name.clone();
...
fn register_or_update_discovered_peer(
    peer: &DiscoveredPeer,
    state: &mut crate::ipc_server::AgentState,
) {
    let known_store = crate::session::load_known_peers().unwrap_or_default();
    let is_paired = known_store.contains(&peer.id)
        || known_store.peers.iter().any(|p| p.name == peer.name);

    if let Some(existing) = state
        .peers
        .iter_mut()
        .find(|p| p.address == peer.address || p.name == peer.name)
    {
                ...
                cfg.peers
                    ...
                    .find(|p| {
                        p.address.as_deref().unwrap_or("").contains(&peer.address)
                            || p.address
                                .as_deref()
                                .is_some_and(|a| !a.is_empty() && peer.address.contains(a))
                            || p.id == peer.id
